...
int find_empty_effect_col(short *dstrow) {
  for (int col=0;col<5;col++) {
    if (dstrow[DIV_PAT_FX(col)] == -1) {
      return col;
    }
  }
  return -1;
...
bool DivEngine::loadVT2(unsigned char* file, size_t len) {
  ...
  try {
        ...
        if (headerType == "PlayOrder") {
            ...
            for (int i=0; i<(int)headerVal.length(); i++) {
                ...
                if (pat_char == 'L') {
                    // loop marker
                    pat_ind_pos=i+1;
                } else if (pat_char == ',' || i == (headerVal.length()-1)) {
                    // delimiter
                    //if (i!=0) {
                    if (i == (headerVal.length()-1))
                      pat_inds.push_back(atoi(headerVal.substr(pat_ind_pos,headerVal.length()-pat_ind_pos).c_str()));
                    else
    ...
    for(int i=0; i<chCount; i++) {
      ...
      ds.subsong[0]->pat[i].effectCols=5;
    }
    ...
    for (int pat=0; pat<ordCount; pat++) {
      ...
      for (int row=0; row<256; row++) {
        ...
        if (line.length() == 0 && row != 0) {
          short* dstrow=chpats[0]->newData[row-1];
          int effect_col = find_empty_effect_col(dstrow);
          if (effect_col != -1) {
            dstrow[DIV_PAT_FX(effect_col)]=0x0D;
            dstrow[DIV_PAT_FXVAL(effect_col)]=0x00;
          }
          break;
        }

        for (int ch=0; ch<chCount; ch++) {
          short* dstrow=chpats[ch]->newData[row];
          ...
          if (ch == 2) {
            ...
            if (env_pitch_str != "....") {
              ...
              int effect_col = find_empty_effect_col(dstrow);
              if (effect_col != -1) {
                dstrow[DIV_PAT_FX(effect_col)]=0x23;
                dstrow[DIV_PAT_FXVAL(effect_col)]=pitch_lo;
              }
              effect_col = find_empty_effect_col(dstrow);
              if (effect_col != -1) {
                dstrow[DIV_PAT_FX(effect_col)]=0x24;
                dstrow[DIV_PAT_FXVAL(effect_col)]=pitch_hi;
              }
            }
          } else if (ch == 0) {
            String env_pitch_str = line.substr(0,4);
            if (env_pitch_str != "....") {
              int effect_col = find_empty_effect_col(dstrow);
              if (effect_col != -1) {
                dstrow[DIV_PAT_FX(effect_col)]=0x25;
                dstrow[DIV_PAT_FXVAL(effect_col)]=0x00;
              }
            }
          }
          ...
          if (chpat_line.at(0) == 'R') {
            dstrow[DIV_PAT_NOTE]=DIV_NOTE_OFF;
            ...
            if (note_char >= 'A' && note_char <= 'G') {
              ...
              dstrow[DIV_PAT_NOTE]=((short)note)+60;
              int effect_col = find_empty_effect_col(dstrow);
              if (effect_col != -1) {
                if (effect == '.') {
                    dstrow[DIV_PAT_FX(effect_col)]=0x01;
                    dstrow[DIV_PAT_FXVAL(effect_col)]=0x00;
                } else if (!(effect >= '1' && effect <= '3')) {
                    dstrow[DIV_PAT_FX(effect_col)]=0x01;
                    dstrow[DIV_PAT_FXVAL(effect_col)]=0x00;
                } else {
                    dstrow[DIV_PAT_FX(effect_col)]=0xF5;
                    dstrow[DIV_PAT_FXVAL(effect_col)]=0x01;
          ...
          // effects
          unsigned char effect_mul = 1;
          if (chpat_line.at(9) != '.') {
            char effect = chpat_line.at(9);
            unsigned char effect_type = 0;
            switch (effect) {
              case '1': {
                effect_type=0x02;
                break;
              ...
              case '9': {
                effect_type=0x26;
                // why does furnace do fractional pitch slides even when NON LINEAR PITCH IS USED!?!?
                effect_mul=7; 
                break;
              }
              case 'A': {
            ...
            effect_val *= effect_mul;
            int effect_col = find_empty_effect_col(dstrow);
            if (effect_col != -1 && effect_type != 0) {
              dstrow[DIV_PAT_FX(effect_col)] = effect_type;
              if (chpat_line.at(10) != '.' && effect != 'B') {
                unsigned char delay_val=VT2_hextoint_env(chpat_line.at(10));
                if (delay_val == 0) dstrow[DIV_PAT_FXVAL(effect_col)] = 0;
                else dstrow[DIV_PAT_FXVAL(effect_col)] = effect_val>>(delay_val-1);
              } else {
                dstrow[DIV_PAT_FXVAL(effect_col)]=effect_val;
              }
            }
          }
          ...
          if (chpat_line.at(5) != '.') {
            ...
            if (env_val > -1) {
              ...
              if (env_val != 15) {
                if (effect_col != -1) {
                  dstrow[DIV_PAT_FX(effect_col)]=0x22;
                  dstrow[DIV_PAT_FXVAL(effect_col)]=env_val<<4;
                }
                dstrow[DIV_PAT_INS]=ins[ch]=ins[ch]|1;
              } else {
                dstrow[DIV_PAT_INS]=ins[ch]=ins[ch]^(ins[ch]&1);
              }
              ord[ch] = 32; // huh?
              do_ins = true;
            }
          ...
          if (chpat_line.at(7) != '.') dstrow[DIV_PAT_VOL]=VT2_hextoint(chpat_line.at(7));
          if (chpat_line.at(6) != '.') {
          ...
          if (do_ins) {
            ...
            dstrow[DIV_PAT_INS]=ins[ch]=(ins_ind<<1)|(ins[ch]&1);
          }
          ...
          if (has_macro_disable[ch] > 0) {
            if ((--has_macro_disable[ch]) == 0) {
              int effect_col = find_empty_effect_col(dstrow);
              if (effect_col != -1) {
                dstrow[DIV_PAT_FX(effect_col)]=0xF6;
                dstrow[DIV_PAT_FXVAL(effect_col)]=0x01;
              }
            }
          }
